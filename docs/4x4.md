...
You may have noticed that there are many way to determine which section will play next. So - what's the best way? Of course there is no single answer for this.

If you are building up a song structure (**Arrange mode**), then the repetition counts on the pads will almost certainly be the main way to determining then each section plays.

While building up arrangements you will probably be clicking on pad to select that section across all 4 tracks. That way you can easily audition the section you are working on.

If you are picking section "on the fly" (operating in **Improv mode**) than you will find that some of the other methods of selection sections are very useful.
...
If you have a MIDI keyboard you easily connect it to the Sel CV/Gate via the MIDI-CV module. by changing the **CV select base octave** in 4X4 you can map all 16 pads to the bottom octave and change of your keyboard. This has the advantage of working with any keyboard, and not requiring much patching in VCV.
...
You can also map the pads to send a sequential series of MIDI note pitches and treat that like a MIDI keyboard with the the VCV MIDI-CV module.
