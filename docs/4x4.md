...
One way to check out the next/prev feature is to use the ZZC Polygate to drive the CV input. If you do this, clicking to 1 button (twice) will queue the next clip, the the 2 button the previous. To try the "goto" feature on channel 3, patch a VCV merge into the 4X4 CV input, the put some voltage source into the '3' input on the merge module.
